...
WindTunnel:
	...
	bne.s	++	; don't interact with wind tunnels while in debug mode
	lea	WindTunnelsCoordinates(pc),a2
...
-	; check for current wind tunnel if the main character is inside it
	...
	blo.s	WindTunnel_Leave	; branch, if main character is too far left
	cmp.w	windtunnel_max_x_pos(a2),d0
	bhs.s	WindTunnel_Leave	; branch, if main character is too far right
	move.w	y_pos(a1),d2
	cmp.w	windtunnel_min_y_pos(a2),d2
	blo.s	WindTunnel_Leave	; branch, if main character is too far up
	...
	subq.w	#4,x_pos(a1)	; move main character to the left
	move.w	#-$400,x_vel(a1)
	move.w	#0,y_vel(a1)
