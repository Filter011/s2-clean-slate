...
SlotMachine_Routine1:
	...
	moveq	#bytesToLcnt($12),d0		; 18 bytes, in words
	moveq	#0,d1
-	move.l	d1,(a1)+
	dbf	d0,-
...
SlotMachine_Routine3:
	moveq	#7,d0				; Only want last 3 bits
	and.b	(Vint_runcount+3).w,d0		; 'Random' seed
	addi.b	#$2C,d0				; ...then add $2C
	...
	moveq	#7,d0				; Only want last 3 bits
	and.b	(Vint_runcount+2).w,d0		; 'Random' seed
	addi.b	#$2C,d0				; ...then add $2C
	...
	moveq	#0,d0
	move.b	d0,slot_index(a4)			; => SlotMachine_Subroutine1
	move.b	d0,slot1_rout(a4)			; => SlotMachine_Routine5_1
	move.b	d0,slot2_rout(a4)			; => SlotMachine_Routine5_1
	move.b	d0,slot3_rout(a4)			; => SlotMachine_Routine5_1
	addq.b	#4,slot_rout(a4)		; => SlotMachine_Routine4
	move.b	(Vint_runcount+3).w,d0		; 'Random' seed
	ror.b	#3,d0				; Mess it around
	lea	SlotTargetValues(pc),a2
...
+
	...
	lea	SlotSequence1(pc),a1		; Slot sequence array for slot 1
	...
	andi.w	#7,d1				; Want only last 3 bits
	lea	SlotSequence2(pc),a1		; Slot sequence array for slot 2
	...
	lea	SlotSequence3(pc),a1		; Slot sequence array for slot 3
...
+
	...
	moveq	#$F,d0				; Want only low nibble
	and.b	(Vint_runcount+3).w,d0		; 'Random' seed
...
+
	...
	lea	SlotSequence1(pc),a3		; Get pointer to slot sequences
...
SlotMachine_Routine6:
	moveq	#0,d0
	move.w	d0,slot1_speed(a4)			; Stop slot 1
	move.w	d0,slot2_speed(a4)			; Stop slot 2
	move.w	d0,slot3_speed(a4)			; Stop slot 3
	move.b	d0,slot_timer(a4)			; Stop drawing the slots
...
SlotMachine_DrawSlot:
	...
	lea	slots_data(a4),a1		; a1 = pointer to slots data
	adda.w	d0,a1				; a1 = pointer to current slot data
	lea	SlotSequence1(pc),a3		; Get slot sequence
...
-	move.l	$80(a2),$80(a1)			; Copy pixel row for second column
	...
	bsr.s	SlotMachine_GetPixelRow		; Need pointer to next pixel row
+
...
SlotMachine_GetReward:
	...
	move.w	SlotRingRewards(pc,d0.w),d0			; Get ring reward
	rts
; ===========================================================================
;loc_2C3D8
SlotMachine_QuadrupleUp:
	add.w	d0,d0				; Quadruple reward
; ===========================================================================
;loc_2C3DC
