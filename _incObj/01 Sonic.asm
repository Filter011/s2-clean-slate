...
+
	...
	move.b	#AniIDSonAni_Stop,anim(a0)	; use "stopping" animation
	bset	#status.player.x_flip,status(a0)
	moveq	#SndID_Skidding,d0	; use "stopping" sound
	jsr	(PlaySound).w
	cmpi.b	#12,air_left(a0)
...
Sonic_Jump:
	...
	beq.s	return_1AA36	; if not, return
	moveq	#0,d0
	move.b	angle(a0),d0
	addi.b	#$80,d0
	bsr.w	CalcRoomOverHead
	cmpi.w	#6,d1			; does Sonic have enough room to jump?
	blt.s	return_1AA36		; if not, branch
	move.w	#$680,d2
	tst.b	(Super_Sonic_flag).w
	beq.s	+
...
Sonic_CheckGoSuper:
	...
	moveq	#SndID_SuperTransform,d0
	jsr	(PlaySound).w	; Play transformation sound effect.
	moveq	#MusID_SuperSonic,d0
	jmp	(PlayMusic).w	; load the Super Sonic song and return
...
+
	...
	moveq	#SndID_SpindashRelease,d0	; spindash zoom sound
	jsr	(PlaySound).w
...
; ---------------------------------------------------------------------------
; Subroutine to slow the Player walking up a slope
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
; Subroutine to push the Player down a slope while they're rolling
; ---------------------------------------------------------------------------
...
SAnim_WalkRun:
	...
	subq.b	#1,d0
+
	moveq	#1<<status.player.x_flip,d2	; is Sonic mirrored horizontally?
	and.b	status(a0),d2
	bne.s	+				; if yes, branch
...
+
	...
	lea	SonAni_Walk(pc),a1	; if yes, branch
	add.b	d0,d0
+
	add.b	d0,d0
	move.b	d0,d3
...
; loc_1B4AE:
SAnim_Super:
	lea	SupSonAni_Run(pc),a1	; use fast animation
	cmpi.w	#$800,d2		; is Sonic moving fast?
	bhs.s	SAnim_SuperRun		; if yes, branch
	lea	SupSonAni_Walk(pc),a1	; use slower animation
	add.b	d0,d0
	add.b	d0,d0
...
SAnim_Tumble:
	move.b	flip_angle(a0),d0
	moveq	#0,d1
	moveq	#1<<status.player.x_flip,d2
	and.b	status(a0),d2
