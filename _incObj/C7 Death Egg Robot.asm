...
loc_3D9D6:
	...
	moveq	#$3F,d0
-	jsr	(Pal_FadeToWhite.UpdateColour).w
	dbf	d0,-
	movea.l	a1,a0
	rts
; ---------------------------------------------------------------------------
...
-	move.l	d0,(a1)+
	...
	moveq	#signextendB(MusID_FadeOut),d0
	jsr	(PlayMusic).w
	move.b	#GameModeID_SegaScreen,(Game_Mode).w ; => EndingSequence
	jmp	(DeleteObject).l
; ===========================================================================
...
loc_3DFBA:
	moveq	#1,d0	; limit to every 2 frames
	and.b	(Vint_runcount+3).w,d0
	bne.s	+	; rts
	jsr	(AllocateObject).l
	bne.s	+	; rts
	move.b	#ObjID_BossExplosion,id(a1) ; load obj
...
loc_3E252:
	...
	addq.b	#1,d1
	move.b	(a1,d1.w),d0
	jsr	(PlaySound).w ; sound id most likely came from off_3E40C or off_3E42C
	addq.b	#1,d1
