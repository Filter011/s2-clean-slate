...
; loc_30494:
Obj89_Init:
	tst.w	(KosPlus_modules_left).w			; is art finished loading?
	beq.s	+				; if yes, branch
	rts
; ---------------------------------------------------------------------------
+
...
Obj89_Main_Sub0:
	...
	addq.b	#2,boss_routine(a0)	; => Obj89_Main_Sub2
...
Obj89_Main_Sub2_AtTarget:
	addq.b	#2,boss_routine(a0)	; => Obj89_Main_Sub4
...
+
	subq.w	#1,(Boss_Countdown).w		; decrement counter
...
Obj89_Main_HandleHoveringAndHits:
	...
	moveq	#SndID_BossHit,d0		; play "boss hit" sound
	jsr	(PlaySound).w
...
Obj89_Main_SubA_Ascend:
	subq.w	#8,(Boss_Y_vel).w		; ascend slowly
	bra.s	Obj89_Main_SubA_Standard
; ===========================================================================
; loc_3092A:
...
Obj89_Pillar_Sub0:
	...
	bne.s	+
	moveq	#SndID_Rumbling2,d0		; play rumbling sound every 32 frames
	jsr	(PlaySound).w
+
	subq.w	#1,y_pos(a0)			; raise pillar
...
Obj89_Pillar_Sub4:
	...
	move.w	(sp)+,y_pos(a0)
	move.b	#1,(Screen_Shaking_Flag).w	; make screen shake
	addq.w	#1,y_pos(a0)			; lower pillar
...
Obj89_Pillar_SolidObject:
	...
	move.w	x_pos(a0),d4
	move.w	y_pos(a0),-(sp)
	addq.w	#4,y_pos(a0)			; assume a slightly lower y position
	jsr	(SolidObject).l
...
Obj89_Arrow_Sub2_Stop:
	addq.b	#2,obj89_arrow_routine(a0)	; => Obj89_Arrow_Sub4
	move.w	d0,x_pos(a0)			; update position
	moveq	#SndID_ArrowStick,d0
	jsr	(PlaySound).w
	jmp	(DisplaySprite).l
; ===========================================================================
; loc_30C7E:
...
Obj89_Arrow_Sub4:
	...
	btst	#status.npc.no_balancing,status(a0)
	beq.s	+
	addq.b	#2,obj89_arrow_routine(a0)	; => Obj89_Arrow_Sub6
+
	bsr.w	Obj89_Arrow_Platform
	lea	Ani_obj89_a(pc),a1
	jsr	(AnimateSprite).l
	jmp	(DisplaySprite).l
; ===========================================================================
; loc_30CAC:
Obj89_Arrow_Sub6:
	...
	bgt.s	JmpTo55_DeleteObject		; if yes, branch
...
Obj89_Arrow_Platform_Decay:
	subq.w	#1,obj89_arrow_timer(a0)	; decrement timer
...
Obj89_Arrow_ChkDropPlayers:
	...
	beq.s	return_30D2A			; branch, if Sonic wasn't standing on the arrow
